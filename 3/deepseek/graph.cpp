...
bool Graph::topologicalSort(vector<int>& result, string& error) const {
    ...
    // Обрабатываем вершины в порядке убывания номеров
...
bool Graph::dfsTopologicalSort(int vertex, vector<bool>& visited, 
    ...
    // Проверяем всех соседей (в порядке убывания номеров вершин)
    for (int j = vertexCount - 1; j >= 0; --j) {
        if (adjacencyMatrix[vertex][j] != 0) { // Есть ребро vertex -> j
            if (!visited[j]) {
